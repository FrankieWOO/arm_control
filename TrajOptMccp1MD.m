classdef TrajOptMccp1MD
    ...
    methods (Static)
        ...
        function [ traj ] = reachFastIlqr( robot, x0, xf, varargin )
            ...
            sdt = 0.01;
            tsim = 0:sdt:tf; % simulation timestamp
            usim = scale_controlSeq(traj.u, t, tsim);
            psim.solver = 'rk4';
            psim.dt = sdt;
            xsim = simulate_feedforward(traj.x(:,1), f, usim, psim);
            traj.tsim = tsim;
            traj.usim = usim;
            traj.xsim = xsim;
            traj.qf = qf;
            ...
            traj.power_in = robot.power_in(xsim(:,1:end-1), usim);
            %power_in1= model.power_in1(xsim(:,1:end-1), usim);
            %power_in1p = max(power_in1,0);
            %power_in2= model.power_in2(xsim(:,1:end-1), usim);
            %power_in2p= max(power_in2,0);
            ...
            traj.Pin = max(traj.Esd(1:end-1) + traj.Pout,0);

            traj.Eout = sum(traj.Pout,2)*sdt;
            traj.Ein = sum(traj.Pin,2)*sdt;
            traj.Energyin = sum(traj.power_in, 2)*sdt;
        ...
        function [ traj ] = reachFastIlqr_fixu1( robot, x0, xf, varargin )
            ...
            costpara.robot = robot;
            costpara.tf = tf;
            costpara.w_tf = sfactor*1e2;
            costpara.w_t  = 1e3; %
            costpara.w_e  = 1e2; % 1 is good for spring force
            j = @(x,u,t)TrajOptMccp1MD.jReachFast(x, u, t, costpara);
            %%%
            opt_param = [];
            ...
            traj.Pout = robot.power_out(xsim(:,1:end-1),usim);
            traj.power_in = robot.power_in(xsim(:,1:end-1), usim); % direct calculated on motors
            %power_in1= model.power_in1(xsim(:,1:end-1), usim);
            %power_in1p = max(power_in1,0);
            %power_in2= model.power_in2(xsim(:,1:end-1), usim);
            %power_in2p= max(power_in2,0);
            ...
            traj.Esd = gradient(traj.Es,sdt);
            traj.Pin = max(traj.Esd(1:end-1) + traj.Pout,0);
            traj.Pin_net = traj.Esd(1:end-1) + traj.Pout;
            traj.Eout = sum(traj.Pout,2)*sdt;
            traj.Ein = sum(traj.Pin,2)*sdt;
            traj.Energyin = sum(traj.power_in, 2)*sdt;
            traj.Ein_net = sum( traj.Pin_net, 2)*sdt;
            %traj.Ein1 = sum(power_in1p)*sdt;
            %traj.Ein2 = sum(power_in2p)*sdt;
            traj.Edamp = sum(traj.power_damp,2)*sdt;
            traj.Erege = sum(traj.power_rege,2)*sdt;
        ...
        function [l, l_x, l_xx, l_u, l_uu, l_ux] = jReachFast(x,u,t, para)
            ...
            if (isnan(u))
                % final cost
                if nargout > 1
                    [l, l_x, l_xx] = TrajOptMccp1MD.costQuad(x-para.xf, para.Hf*w_tf);
                else
                    l = TrajOptMccp1MD.costQuad(x-para.xf, para.Hf*w_tf);
                end
            else
                % running cost
                %para = [];
                para.w_t = w_t;
                para.w_e = w_e;
                fl = @(x,u,t)TrajOptMccp1MD.costr_reachFast(x,u,para);
                l = fl(x,u,t);
        ...
        function c = costr_reachFast(x, u, para)
            c1 = (x(1) - para.qf).^2;
            
            %ce = (para.robot.spring_force(x,u)).^2;
            ce = (u(1) - x(1))^2 + u(2)^2 + 1e-6*u(3)^2;
            c = c1*para.w_t/para.tf  + ce*para.w_e ;
        end
